 ...
 * 扣除冻结投标金额，生成待收本金
 */
public class ProjectDecuctFreezeExecuter extends BaseExecuter
{
	...
	{
		// 先取出更新前的account_sum
		AccountSum accountSum = accountSumDao.findByUserId(super.user
				.getUserId());
		// 取出可能扣款的值
		...
		double useHuikuanInterest = BigDecimalUtil.sub(huikuanInterest,
				usedHuikuanInterest);
		...
		if (currHuikuan > 0)
		{
			...
			Map<String, Object> tranData = Global.getTransfer();
			Borrow borrow = (Borrow) (tranData.get("borrow"));

			// 回款规则配置 秒标、净值标、天标投标不使用回款额度，不产生回款续投奖励
			HuikuanConfRuleCheck rule = (HuikuanConfRuleCheck) Global
					.getRuleCheck("huikuanConf");
			if (rule != null)
			{
