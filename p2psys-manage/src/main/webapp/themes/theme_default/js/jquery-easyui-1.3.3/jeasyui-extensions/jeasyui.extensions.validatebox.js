...
(function ($, undefined) {
    ...
    var rules = {
		...
		currencyNum:{
            ...
            message: '请输入小于10位非负整数[小数最多精确到两位]'
		},
        ...
        password: {
            ...
            message: "输入的内容必须是安全的密码字符(由字符和数字组成，6-16 位)格式."
        },
        //  输入的字符必须是指定的内容相同
        equals: {
            validator: function (value, param) {
        ...
        timeLimit:{
            ...
            message: "输入的期限必须在{0}月至{1}月之间"
		},
		timeLimitDay:{
			validator: function (value, param) {
				return (value >= param[0])&&(value <= param[1]); 
            },
            message: "输入的期限必须在{0}天至{1}天之间"
		},
		lowestAccount: {
            validator: function (value, param) { return value >= param[0]; },
            message: "最低投标金额不得小于{0}元."
        },
        upAccount: {
            validator: function (value, param) { return value >= param[0]; },
            message: "最高投标金额不得小于{0}元."
        },
