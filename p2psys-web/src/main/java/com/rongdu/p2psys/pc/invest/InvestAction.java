...
public class InvestAction extends BaseAction<BorrowModel> implements ModelDriven<BorrowModel>
{
	 ...
	 * @throws Exception
	 */
	@Action(value = "/nb/pc/account/addInvest")
	public void addNbInvest() throws Exception
	{
	    ...
	    if(checkInvestToken())
	    {
			...
			Long[] ids = ConstantUtil.getIds(paramString("ids"));//红包ID
			...
			if(ids!=null&&ids.length>0)
			{
				if(ids.length>2)
				{
					checkResult = "最多使用两个红包!";
				}
				else
				{
					if(1==ids.length)
					{
						UserRedPacket uRedPacket = userRedPacketService.findUserRedPacketById(ids[0]);
						if(uRedPacket==null){
							...
							if(uRedPacket.getUser().getUserId()!=getNBSessionUser().getUserId())
							{
								checkResult = "含有非法的红包参数！";
							}
						}						
					...
					else
					{
						...
						if(userRedPacketService.findUserRedPacketById(ids[0]).getType().getServiceType().equals(userRedPacketService.findUserRedPacketById(ids[1]).getType().getServiceType()))
						{
							checkResult = "含有相同的红包类型！";
						}
					}
				}
			}
				...
				else
				{
					if(1==cids.length)
					{
						...
						if(cp==null){
							...
							if(cp.getUserTo().getUserId()!=getNBSessionUser().getUserId())
							{
								checkResult = "含有非法的加息券参数！";
							}
						}
					...
					else
					{
						...
						{
							Coupon cp = couponService.findCouponInfoById(cids[i]);
							if(cp==null){
								checkResult = "含有非法的加息券参数！";
							}else{	
								if(cp.getStatus()!=CouponStatusConstant.STATUS_UNUSED)
			 ...
			 * 校验不成功
			 */
			if (!ConstantUtil.SUCCESS.equals(checkResult))
			{
