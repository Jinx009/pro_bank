 ...
 *
 */
public class CfWechatSessionFilter implements Filter {
	...
	public void doFilter(ServletRequest request, ServletResponse response,
		...
		if (!pathList.contains(servletPath)) {
			if (sessionUser == null) {
				...
				httpServletResponse
						.sendRedirect("/cf/wechat/login.html?timeout=1&redirectUrl="
								+ redirectURL);
				return;
			} else {
	...
	private List<String> notNeedSessionCheck() {
		String[] paths = new String[] { "/wechat/pro/list.html",
				"/wechat/pro/del.action",
				"/wechat/pro/del.html",
