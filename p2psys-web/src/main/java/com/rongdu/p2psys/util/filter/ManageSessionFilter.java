 ...
 *
 */
public class ManageSessionFilter implements Filter {
	...
	public void doFilter(ServletRequest request, ServletResponse response,
		...
		User sessionUser = (User) session.getAttribute(ConstantUtil.SESSION_USER);
		...
		if (sessionUser == null) {
			...
			httpServletResponse.sendRedirect("/cf/login.html?timeout=1&redirectUrl="+redirectURL);
			return;
		} else {
			if (!pathList.contains(servletPath)){
				UserCache userCache = sessionUser.getUserCache();
				if(null==userCache.getApprovalStatus()||-1==userCache.getApprovalStatus()){
					httpServletResponse.sendRedirect("/manage/code/index.html");
					return;
				}
			}
		}
