...
Handlebars.registerHelper("transFormatMobilePhone",function(value){
	
	return value.toString().substring(0,3)+"****"+value.toString().substring(7,11);
...
//--------------------我的借款-我的借款----------------------------
//标题
Handlebars.registerHelper("logBorrowNameFun",function(name,id,addTime,typeStr){
	var url = "";
	if(typeStr=="固定收益类产品"){
		url = "/invest/detail.html";
	}
	if(typeStr=="海外投资产品"){
		url = "/invest/detail.html";
	}
	...
	if(name.length > 10){
		return new Handlebars.SafeString("<a href='"+url+"?id="+id +"&startTime="+addTime+"' title="+name+" target='_blank'>"+name.substring(0,10)+"…</a>");
	}else{
		return new Handlebars.SafeString("<a href='"+url+"?id=" + id + "&startTime="+addTime+"' title=" + name + " target='_blank'>" + name + "</a>");
	}
})

Handlebars.registerHelper("logBorrowFun",function(name,id,addTime,typeStr,flagId){
	var url = "";
	if(typeStr=="固定收益类产品"){
		url = "/invest/detail.html";
	}
	...
	if(name.length > 10){
		return new Handlebars.SafeString("<a href='"+url+"?id="+id +"&startTime="+addTime+"&flagId="+flagId+"' title="+name+" target='_blank'>"+name.substring(0,10)+"…</a>");
	}else{
		return new Handlebars.SafeString("<a href='"+url+"?id=" + id + "&startTime="+addTime+"&flagId="+flagId+"' title=" + name + " target='_blank'>" + name + "</a>");
	}
})


Handlebars.registerHelper("logBorrow1Fun",function(name,id,addTime,borrowType,flagId){
...
Handlebars.registerHelper("logBorrowTypeFun",function(name,id,addTime,borrowType){
	var url = "";
	if(borrowType==103){
		url = "/invest/detail.html";
	}
...
Handlebars.registerHelper("ppfundBookingOut",function(outTime,isFixedTerm){
	...
	if(isFixedTerm == 1){
		...
		return new Handlebars.SafeString('<a class="zc">转出</a>');
	}
})
...
Handlebars.registerHelper("ppfundAccountWait",function(account, accountYes){
	return (account-accountYes).toFixed(2);
})
...
Handlebars.registerHelper("indexContent1",function(content){
	var content = content.replace(/<[^>].*?>/g,"");
	content = content.replace(/&nbsp;/g,"");
	if(content.length >200)
	{
		return new Handlebars.SafeString(content.substr(0,200)+'…');
	}
	else
	{
		return content;
...
Handlebars.registerHelper("moneyFormat1",function(money){
	if(money == 0)
	{
		return money;
	}
		...
		var timer = setInterval(function(){
	        if (sys_second > 301) {
	          ...
	          $("#djs").html('剩余时间：'+showDay+'天'+showHour+'时'+showMinute+'分'+showSecond+'秒')
		        	...
		        	clearInterval(timer);
		        	$("#dsj-detail").css("display","none");
		        	$(".endtime-djs").css("display","block")
		        	var endtimer = setInterval(function(){
		        		if (sys_second > 1) {
				          ...
				          var showMinute = minute<10?"0"+minute:minute;//计算分钟
				          var showSecond = second<10?"0"+second:second;//计算秒杀
				          $("#enddjs").html('剩余时间：'+showDay+'天'+showHour+'时'+showMinute+'分'+showSecond+'秒')
		        		}
		        		else{
		        			location.reload();
		        		}
...
//判断逾期借款时间
Handlebars.registerHelper("lesstime",function(expirationTime,options){
	var time = parseInt((new Date()).valueOf());
	/*var vtime = parseInt(vtime);
	var days = parseInt(days)*24*60*60*1000;*/
    var alltime = parseInt(expirationTime);
   console.log(time)
   console.log(alltime)
   if(alltime < time){
     //满足添加继续执  行
     ...
     //不满足条件执行{{else}}部分
        var sys_second =(alltime-time)/1000;
		var timer = setInterval(function(){
	        if (sys_second > 1) {
	          sys_second -= 1;
	          var day = Math.floor((sys_second / 3600) / 24);
	          ...
	          var showMinute = minute<10?"0"+minute:minute;//计算分钟
	          var showSecond = second<10?"0"+second:second;//计算秒杀
	          $("#dtime").html('剩余时间：'+showDay+'天'+showHour+'时'+showMinute+'分'+showSecond+'秒')
	        } else {
	        	 location.reload();
	        }
	      }, 1000);
...
Handlebars.registerHelper("maindoremarkFunction",function(remark){
	if(remark.length > 15){
...
Handlebars.registerHelper("expertDetailContent",function(content){
	if(content == "" || content == null || typeof(content) == 'undefined' || content== 'undefined')
	{
		return ""
	}
	else
...
Handlebars.registerHelper("nameFormat1",function(productName){
		if(productName.length>4){
			return productName.substring(4,productName)+"...";
		}else{
			return productName;
		}
});
