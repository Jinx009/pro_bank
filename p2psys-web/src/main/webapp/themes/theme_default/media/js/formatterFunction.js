...
var  invest_TypeFun = function(value,rows){
	...
	switch (rows.type)
	{
		...
		case 116:
			...
			break;
		case 119:
			showVal = "estate.png";//海外投资产品
			type_title = "海外投资产品";
			break;
		case 122:
			showVal = "entrust.png";//浮动收益类产品
			type_title = "浮动收益类产品";
			break;
	}
	...
	if(rows.status == 0){
		if(rows.type == 110){
			...
			return "<a href='/borrow/loan.html?borrowId=" + rows.id + "&type=offvouch'>编辑</a>|" + 
		           '<a href="#" onClick=cancel("/member/borrow/cancel.html?id='+rows.id+'") >撤回</a>';
		}else if(rows.type == 119){
			return "<a href='/borrow/loan.html?borrowId=" + rows.id + "&type=estate'>编辑</a>|" + 
	           '<a href="#" onClick=cancel("/member/borrow/cancel.html?id='+rows.id+'") >撤回</a>';
		}else if(rows.type == 122){
			return "<a href='/borrow/loan.html?borrowId=" + rows.id + "&type=entrust'>编辑</a>|" + 
	           '<a href="#" onClick=cancel("/member/borrow/cancel.html?id='+rows.id+'") >撤回</a>';
		}
...
//我的借款----------END-----------------
var protocolBorrowerOperateFun = function(value,rows){
	if(rows.status == 3 || rows.status == 6 || rows.status == 7 || rows.status == 8){ //添加判断是否满标复审通过
		return "<a href='/member/borrow/borrowerProtocol.html?borrowId=" + rows.id + "' >下载协议</a>";
	}else{
		return "-";
	}
}
...
//	}
	if(rows.status==1){
		...
		return '<a href="#" onClick=repay("/member/borrow/doPriorRepay.html?repaymentId='+rows.id+'") >提前还款</a>--<a href="#" onClick=repay("/member/borrow/repay.html?repaymentId='+rows.id+'") >还款</a>';
	}
}
