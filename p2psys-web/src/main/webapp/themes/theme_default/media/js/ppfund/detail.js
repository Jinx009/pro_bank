    ...
    success:function(json){
      ...
      require.async("/plugins/handlebars-v1.3.0/handlebars-v1.3.0",function(){
          require.async("/plugins/handlebars-v1.3.0/transFormatJson",function(){
            $("#invest-main-box").html(Handlebars.compile(require("../../tpl/ppfund/detail.tpl"))(json));
        	...
        	require.async('common1',function(){
             ...
             $(".all-tb").click(function(){
               ...
               $("#money").val(okmoney);
               if(isNaN(okmoney)){
            	   okmoney = $(".pageUseMoney").html().replace(/,/gm, "");
            	   $("#money").val(okmoney);
            ...
            var 
                  ...
                  $money = $("#money"),//输入投资金额
                  $userCoinMoney = $("#userCoinMoney"),//输入果果币
                  $borrowVoucherMoney = $("#borrowVoucherMoney"),//输入抵价券
            ...
            $(".sub-invest-btn").click(function(){
              ...
              var $form_out_time_tip = $(".form_out_time_tip");

            	  var  money= $money.val();
                  if($money.val() == "" || $money.val() == 0)
                  {
                	  $form_mony_tip.removeClass("hide").html('请输入投资金额！');
                      return false;
                  }

                  if((parseFloat(json.ppfund.account) != 0) && json.accountWait > parseFloat(json.ppfund.lowestAccount)||parseFloat(json.ppfund.account) == 0)
                  {
                      if(money < parseFloat(json.ppfund.lowestAccount))
                      {
                        $form_mony_tip.removeClass("hide").html('投资金额不能小于最小投标金额<a href="javascript:;" id="invest_all">'+json.ppfund.lowestAccount+'</a>元！');
                        $("#invest_all").click(function(){
                          $money.val(json.ppfund.lowestAccount);
                      ...
                      else if(money > parseFloat(json.ppfund.account)&&parseFloat(json.ppfund.account)>0)
                      {
                        $form_mony_tip.removeClass("hide").html('投资金额不能大于可投金额');
                        $("#invest_all").click(function(){
                          $money.val(json.ppfund.lowestAccount);
                          $form_mony_tip.addClass("hide");
                          redPacketMoneyTotal();
                      ...
                      else if(money > parseFloat(json.ppfund.mostAccount) && parseFloat(json.ppfund.mostAccount) != 0)
                      {
                        $form_mony_tip.removeClass("hide").html('投资金额不能大于最大投标额度,当前<a href="javascript:;" id="invest_all">可投'+json.ppfund.mostAccount+'</a>!');
                        $("#invest_all").click(function(){
                          $money.val(json.ppfund.mostAccount);
                          $form_mony_tip.addClass("hide");
                          redPacketMoneyTotal();
                        });
                        return false;
                      ...
                      else if((parseFloat(json.ppfund.account) != 0) && money > json.accountWait)
                      {
                        $form_mony_tip.removeClass("hide").html('投资金额不能大于可投金额，可点击<a href="javascript:;" id="invest_all">全部投满</a>剩余金额!');
                        $("#invest_all").click(function(){
                            $money.val(parseFloat(json.ppfund.account) - parseFloat(json.ppfund.accountYes));
                            $form_mony_tip.addClass("hide");
                            redPacketMoneyTotal();
                        });
                        return false;
                      ...
                      if(money > parseFloat(json.account.useMoney)&&json.ppfund.pidNo!=="体验标")
                      {
                    	 $form_mony_tip.removeClass("hide").html('账户余额不足，请<a href="/member/recharge/newRecharge.html" target="_blank">立即充值</a>!');
                      }else if(0 >= parseFloat(json.userInvestIdentify.realNameStatus)&&json.ppfund.pidNo==="体验标")
                      {
                     	 $form_mony_tip.removeClass("hide").html('您未实名认证，请<a href="/member/recharge/newRecharge.html" target="_blank">立即认证</a>!');
                       }
                      else
                      {
                        ...
                        require.async(['/plugins/layer-v1.8.4/skin/layer.css','/plugins/layer-v1.8.4/layer.min'],function(){
                						...
                						$.layer({
                						    ...
                						    area: ['460px', '275px'],
                						    page: {
                						        html: '<div class="fileconfirm"><div class="fileconfirm_msg"><p>您正在进行投资交易，投资完成前，请不要关闭本窗口。投资完成后，请根据您的投资结果点击下面按钮。</p></div><div class="fileconfirmbtn"><a href="javascript:;" class="okBtn reFreshWin">投资成功</a><a href="javascript:;" class="cancleBtn closeBtn reFreshWin">投资失败</a></div></div>'
                						    },
                						    close:function(){
                  ...
                  //账户余额小于可投金额
                  else if(parseFloat(json.account.useMoney) < json.accountWait&&!json.ppfund)
                  {
                      $(".form_group_tip").removeClass("hide").html('账户余额不足，请<a href="/member/recharge/newRecharge.html" target="_blank">立即充值</a>!');
                  }
                  else if(parseFloat(json.account.useMoney) < json.accountWait&&json.ppfund&&json.ppfund.pidNo!=="体验标")
                  {
                      $(".form_group_tip").removeClass("hide").html('账户余额不足，请<a href="/member/recharge/newRecharge.html" target="_blank">立即充值</a>!');
                  }
                  else if(((json.ppfund.account) != 0) && money != json.accountWait)
                  {
                      $(".form_group_tip").removeClass("hide").html('由于当前可投金额小于最小投标额度，只能<a href="javascript:;" id="invest_all">全部投满</a>剩余金额!');
                        $("#invest_all").click(function(){
                          $money.val((parseFloat(json.ppfund.account) - parseFloat(json.ppfund.accountYes)));
                          $form_mony_tip.addClass("hide");
                          redPacketMoneyTotal();
            ...
            require.async(["commonJS/keep","commonJS/rollTo"],function(){
              ...
              $(".detail-menus").keep(function(){
                ...
                if($(".detail_menus_center a").hasClass("a1"))
                {
            ...
            require.async(['/plugins/pager/pager.css','/plugins/pager/pager'],function(){
            	kkpager.generPageHtml({
                  ...
                  click:function(n){
                        $.ajax({
                          type:"get",
                          url:"/ppfund/ppfundInForJson.html?random="+Math.random()+"&page="+n+"&id="+$("#pid_id").val(),
                          dataType:"json",
                          success:function(json){
