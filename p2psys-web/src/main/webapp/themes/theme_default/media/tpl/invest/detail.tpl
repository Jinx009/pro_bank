 ...
 <div class="invest-main-left">
    <div class="bid_info mb20 bdr1-e6e6e6 pt40 pb30 bg-white">
      ...
      <ul class="bdr1-e6e6e6 pt10 pb10 pl20 pr20">
        ...
        <li>最多投标金额：<span>￥{{moneyFormat mostAccount}}</span></li>
  ...
  <div class="invest-main-right" id="tz-detail">
    <div class="mb20 bdr1-e6e6e6 bg-white">
      <h3 class="invest-rightside-h3">
          投资金额
          {{#inequality borrow/scales 100}}
            {{#lesstime borrow.addTime borrow.validTime}}
              <div class="dtime"></div>
            {{else}}
               <div id="dtime"></div>
            {{/lesstime}}
		...
		<div class="amount-form-box pt10 pl20 pr20" style="padding-left:30px;">
              ...
              <input type="hidden" name="borrowId" value="{{../borrow.id}}" id="borrowId">
		          <div class="form-group">
		             投标金额：<input type="text" id="money" autocomplete="off" name="money" size="11"  placeholder="输入投标金额" onkeyup="if(this.value==this.value2);if(this.value.search(/^\d*(?:\.\d{0,2})?$/)==-1)this.value=(this.value2)?this.value2:'';else this.value2=this.value;" style="width:170px;border:1px solid #f63"/>
                ...
                <span class="all-tb">全投</span>
                    ...
                    {{!S红包dom结构}}
                      <div  id="choiceRedPacket" class="hide choiceRedPacketBox" style="background:#ffffff;height:420px;overflow-y:scroll;">
                        ...
                        {{#each ../packets}}
                          ...
                          <label for="choiceRedPacket{{@index}}">
                          	<p  class="redPacketName">{{name}}</p>
	                    		<p class="redPacketMoneyP">￥<i class="redPacketMoney">{{amount}}</i>
								<input type="checkbox" name="ids" value="{{id}}" id="choiceRedPacket{{@index}}"/>
						    	</p>
						...
						{{#each ../spackets}}
                          <li>
	                          <label for="radioRedPacket{{@index}}">
	                          	<p  class="redPacketName">{{name}}</p>
	                            <p class="redPacketMoneyP">￥<i class="redPacketMoney">{{amount}}</i>
								<input type="radio" name="redPacketId" value="{{id}}" checked=false  id="radioRedPacket{{@index}}"/>
						    	</p>
	                          	<p class="expirationTimeP"> {{dateFormat expiredTime}}过期</p>
	                          </label>
                          </li>
                        {{/each}}
                         <li style="display:none"><input type="radio" name="redPacketId" value="0" checked=true  id="radioRedPacketHide"/> </li>
                        </ul>
      ...
      <div class="amount-form-box borrow_full pt10 pl30 pr30">
        ...
        <p>欢迎进入<a href="/nb/pc/product/productList.html?id=" id="investDetail"}>投资列表</a>，投资其他借款。</p>
      </div>
      {{/with}}
      <input type="hidden" id="bid_id" value="{{../borrow/id}}" />
      {{/inequality}}
      ...
      <div class="mb20 bdr1-e6e6e6 bg-white" style="border:0;">
       ...
       <p class="amount-money">账户余额：<b class="c315A8B">￥<em class="pageUseMoney">{{moneyFormat1 useMoney}}</em></b><br/>
            ...
            {{moneyFormat1 ../sumBidMoney}}</span></b></p>
              <div class="form-group">
                 预约金额：<input type="text"  readonly="readonly" autocomplete="off" name="" size="11"  placeholder="输入投标金额" style="width:170px;border:1px solid #f63"/>
                        <div class="form_group_tip form_mony_tip hide">请输入投标金额进行投标</div><i>元</i>
              </div>
             {{#equal ../../payPwd false}}
              <div class="form-group">
                交易密码：<input type="password" readonly="readonly"  name="" size="11" placeholder="交易密码" autocomplete="off" />
                  <div class="form_group_tip form_trade_tip hide"></div>
                   <i class="lock"></i>
              </div>
              {{else}}
           ...
           {{/equal}}
             {{#if ../isDirectional}}
                      <div class="form-group">
                          定向密码：<input type="password"  autocomplete="off" name="" size="11" placeholder="输入定向密码" style="width:180px; border-radius:4px;"/>
                          <div class="form_group_tip form_directional_tip hide"></div>
                          <i class="lock"></i>
                      </div>
                      {{/if}}
    ...
    <div class="mb20 bdr1-e6e6e6 bg-white" style="width:1190px; margin:0 auto;">
      <div class="detail-menus clearfix">
        <div class="detail_menus_center">
          ...
          {{#if borrowUploads}}<a href="javascript:void(0)" class="a4">项目资料</a>{{/if}}
        ...
        <div class="borrow-detail mb30">
          ...
          <div class="tab-content" style="width:800px; margin:50px auto 0;">
            <div class="album">
              ...
              {{#each borrowUploads}}
                {{#equal type 4}}
                  <li><a href="{{picPath}}" class="fancybox" ><img src="{{picPath}}" alt="项目资料"></a></li>
                {{else}}
                  <li><a href="{{../../url}}{{picPath}}" class="fancybox" ><img src="{{../../url}}{{picPath}}" alt="项目资料"></a></li>
                {{/equal}}
              {{/each}}
              </ul>
