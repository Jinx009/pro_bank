define(function(require,exports,module){
	    					    		...
	    					    		success:function(json){
	    					    			...
	    					    			if(jsonObj.result == false)
	    					    			{
	    					    				$.layer({
				    							    ...
				    							    border: [1, 1, '#cecfd0'],
				    							    page: {
				    							        html: '<div class="tipsWrap w384"><div class="tipsTxt"><i class="iconfont okIco">&#xe63d;</i><span>交易密码设置成功！</span></div><br/><a href="/nb/pc/recharge/newRecharge.html" class="bank-btn">立即前往充值</a></div>'
				    							    },
				    							    close: function(index){
				    							    	window.location.reload();
				    							    },
	    					    			...
	    					    			else
	    					    			{
	    					    				$.layer({
				    							    ...
				    							    time:3,
				    							    page: {
				    							        html: '<div class="tipsWrap w384"><div class="tipsTxt"><i class="iconfont okIco">&#xe63d;</i><span>交易密码修改成功！</span></div><div class="tipsMsg">3秒后窗口自动关闭</div></div>'
				    							    },
				    							    close: function(index){
				    							    	window.location.reload();
				    							    },
		...
		if($('#email').val()!="" ){
			...
			$.ajax({
				...
				success:function(data){
					if(data){
						$.ajax({
							url:'/nb/pc/security/bindEmailCode.html?email='+$('#email').val(),
							type:'post',
							success:function(data){
								if(data.result){
									get_code_time(o);
	...
	$("#getHavePhoneVcode").click(function(){
		$.ajax({
			url:'/nb/pc/security/modifyPhoneCode.html?mobilePhone='+$('#userMobilePhone').text(),
			type:'post',
			success:function(data){
				if(data.result){
					var time=60;
