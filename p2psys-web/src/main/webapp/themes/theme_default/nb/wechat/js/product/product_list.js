...
$(function() {
    ...
    $.ajax({
        url: "/product/showProductListByFlag.html?id="+p_id,
        type: 'GET',
        dataType: 'JSON',
        ansyc: false,
        success: function(res) {
                ...
                for (var ii = 0, ll = goods.length; ii < ll; ii++) {
                    ...
                    if (goods[ii].experienceModel) {
                        ...
                        // 产品状态
                        if(parseFloat(goods[ii].experienceModel.scales) < 100){
                            if(goods[ii].hotProduct==1){goods[ii].proStatus=3;}else{goods[ii].proStatus=4;}
                        }else{
                            goods[ii].proStatus=1;
                        }
                        //浮动加息
                    ...
                    if (goods[ii].ppfundModel) {
                        ...
                        // 产品状态
                        if(parseFloat(goods[ii].ppfundModel.scales) < 100){
                            if(goods[ii].hotProduct==1){goods[ii].proStatus=3;}else{goods[ii].proStatus=4;}
                        }else{
                            goods[ii].proStatus=1;
                        }
                    ...
                    if (goods[ii].borrowModel) {
                        ...
                        if(parseFloat(goods[ii].borrowModel.scales) < 100){
                            if(goods[ii].hotProduct==1){goods[ii].proStatus=3}else{goods[ii].proStatus=4}
                        }else{
                            goods[ii].proStatus=1;
                        }
