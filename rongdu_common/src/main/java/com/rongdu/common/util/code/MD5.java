...
public class MD5 {
	...
	// 这里把它们实现成为static final是表示了只读，能在同一个进程空间内的多个Instance间共享。
	...
	// 在java中，实现成了private方法，保持了原来C中的名字。
	...
	/**
	 * 一个内部使用的byte数组的块拷贝函数，从input的inpos开始把len长度的字节拷贝到output的outpos位置开始。
	 */
	...
	/**
	 * 核心变换程序，由md5Update调用，block是分块的原始字节。
	 */
	 ...
	 * 把long数组按顺序拆成byte数组
	 * 
	 * 因为java的long类型是64bit的，只拆低32bit，以适应原始C实现的用途。
	 */
	 ...
	 * 把byte数组按顺序合成成long数组
	 * 
	 * 因为java的long类型是64bit的，只合成低32bit，高32bit清零，以适应原始C实现的用途。
	 */
	...
	/**
	 * 最主要的公共方法，入口参数是你想要进行MD5变换的字符串，返回的是变换完的结果，这个结果是从公共成员digestHexStr取得的．
	 */
